...
public class QiniuWrapper {
    ...
    public  String getUploadToken(String key , boolean update){
         ...
         */
        String fops = "imageView2/0/q/75|watermark/2/text/c2ltcGxlYXN5/font/5qW35L2T/fontsize/480/fill/IzE4OEIyRg==/dissolve/100/gravity/SouthEast/dx/10/dy/10|imageslim" ;
        /**
         *可以对转码后的文件进行使用saveas参数自定义命名，当然也可以不指定文件会默认命名并保存在当前空间。
         * saveBucket+":"+saveKey
         * bucketname为空间的名称，key为保存的文件名
         */
        String urlbase64 = UrlSafeBase64.encodeToString(qiniuConfigProperties.getBucket()+":ANS");
    ...
    public  String upload(byte[] data,String key,boolean update){
        try {
            String token = getUploadToken(key , update);
